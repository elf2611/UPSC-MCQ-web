    ...
    <ProtectedRoute>
      <div className="max-w-7xl mx-auto px-4 py-8 flex flex-col md:flex-row gap-8">
              ...
              <div className="space-y-3">
                {subjects.map(sub => (
                  <label key={sub.id} className="flex items-center gap-3 cursor-pointer group">
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${selectedSubjects.includes(sub.name) ? 'bg-primary border-primary' : 'border-white/20 bg-background group-hover:border-white/40'}`}>
                      {selectedSubjects.includes(sub.name) && <svg className="w-3 h-3 text-primary-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                    </div>
                    <span className="text-sm text-gray-300">{sub.name}</span>
                    <input type="checkbox" className="hidden" checked={selectedSubjects.includes(sub.name)} onChange={() => toggleSubject(sub.name)} />
                  </label>
                ))}
              </div>
            </div>
        ...
        <main className="flex-1">
          ...
          {/* Topic Cards Grid */}
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {topicStats.filter(t => selectedSubjects.includes(t.name)).map(topic => (
                ...
                <div className="mt-auto">
                  ...
                  <div className="w-full h-1.5 bg-background rounded-full overflow-hidden mb-6 border border-white/5">
                    <div 
                      ...
                      style={{ width: topic.totalQs > 0 ? `${(topic.attempted / topic.totalQs) * 100}%` : '0%' }} 
                    />
                  </div>
                  
                  {topic.totalQs > 0 ? (
                    <Link 
                      href={`/test-interface?mode=practice&subject=${encodeURIComponent(topic.name)}&difficulty=${encodeURIComponent(difficulty)}`}
