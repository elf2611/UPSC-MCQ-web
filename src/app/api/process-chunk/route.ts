...
const updateJobStatus = async (jobId: string, message: string) => {
  await supabaseAdmin
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (jobError || !job) {
      return NextResponse.json({ error: 'Job not found or already processing/completed.' }, { status: 404 });
    }
    ...
    try {
      // 3. Download the PDF
      await updateJobStatus(jobId, 'Extracting PDF...');
      ...
      const pdfData = await pdfParse(chunkBuffer);
      ...
      // 5. Send to Gemini
      await updateJobStatus(jobId, 'Generating Questions (AI)...');
      ...
      try {
        ...
        // If it failed due to JSON parse after retries, rawResponse is attached
      ...
      // 6. Validate JSON and check taxonomy
      await updateJobStatus(jobId, 'Validating JSON...');
